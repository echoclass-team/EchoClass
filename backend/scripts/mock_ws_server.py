...
- 1 个教案（数学/分数初步）
- 2 个学生（小明 / 小红），各 2 个问题（共 4 个 dialog）
...
def _student_questions(student_id: str, name: str) -> list[StudentQuestion]:
    return [
        ...
        StudentQuestion(
            ...
            speaker_id=student_id,
            speaker_name=name,
            content="老师，1/2 是不是就是 0.5？为什么要写成这样？",
            category="extend_topic",
            difficulty="medium",
            linked_key_point="分数的读写方法",
            rationale="想把分数和小数联系起来。",
        ),
    ]
