...
class DirectorAgent:
    ...
    async def decide(
        ...
        """根据当前课堂状态返回下一步调度决策。"""
        self._validate_students(students, stage=stage)
    ...
    def _validate_students(
        ...
        sids = [self._sid(student) for student in students]
        if len(set(sids)) != len(sids):
            raise ValueError("student speaker_id must be unique")

        expected_stage_id = stage_id or getattr(stage, "id", None)
        if not expected_stage_id:
            return
        for student in students:
            persona_stage_id = getattr(student, "stage_id", None)
            if persona_stage_id and persona_stage_id != expected_stage_id:
                raise ValueError(
                    "stage/persona stage_id mismatch: "
                    f"stage_id={expected_stage_id}, persona={self._sid(student)} "
                    f"stage_id={persona_stage_id}"
                )
    ...
    @staticmethod
    def _keep_single_speak(actions: list[StudentAction]) -> list[StudentAction]:
    ...
    @staticmethod
    def _dedupe_same_student(actions: list[StudentAction]) -> list[StudentAction]:
        """同一学生同轮多个动作按 speak > raise_hand > daydream > silent 去重。"""
        rank = {"speak": 4, "raise_hand": 3, "daydream": 2, "silent": 1}
        chosen: dict[str, StudentAction] = {}
        for action in actions:
            current = chosen.get(action.speaker_id)
            if current is None:
                chosen[action.speaker_id] = action
                continue
            current_key = (rank.get(current.action_type, 0), current.priority)
            action_key = (rank.get(action.action_type, 0), action.priority)
            if action_key > current_key:
                chosen[action.speaker_id] = action
