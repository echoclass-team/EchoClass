...
from graph.checkpoint import CheckpointStore, InMemoryCheckpointStore
from graph.state import ClassroomState, PendingQuestion
from schemas.director import DirectorDecision, Message, StudentAction
...
def build_classroom_graph(node_mapping: Mapping[str, Callable[[ClassroomState], Any]] | None = None) -> Any:
    ...
    nodes = node_mapping or {node: passthrough for node in ("teacher_input", "director", "fanout_students", "aggregate", "persist", "wait")}
    for node in ("teacher_input", "director", "fanout_students", "aggregate", "persist", "wait"):
...
class ClassroomGraph:
    def __init__(
        ...
        *,
        ...
        self.llm = llm
        self.student_agent_factory = student_agent_factory or self._default_student_agent_factory
    ...
    async def run_turn(self, state: ClassroomState, teacher_utterance: str) -> ClassroomState:
    ...
    async def teacher_input_node(self, state: ClassroomState) -> ClassroomState:
        ...
        state["transcript"].append(Message(role="teacher", speaker_id="teacher", content=teacher_utterance, timestamp_seconds=state["elapsed_seconds"]))
        self._mark_taught_points(state, teacher_utterance)
        return state

    async def director_node(self, state: ClassroomState) -> ClassroomState:
        decision: DirectorDecision = await self.director.decide(state["last_teacher_utterance"] or "", state["stage"], state["students"], state["transcript"], state["elapsed_seconds"])
        state["director_history"].append(decision)
        mapping = {"raise_hand": "hand_raise", "daydream": "distraction", "speak": "student_speak", "silent": "silent"}
        for action in decision.actions:
            state["pending_events"].append(self._next_event(state, DirectorEvent, event=mapping[action.action_type], speaker_id=action.speaker_id, description=f"{action.speaker_id} {action.action_type}", rationale=decision.rationale))
        return state

    async def fanout_students_node(self, state: ClassroomState) -> ClassroomState:
        actions = state["director_history"][-1].actions if state["director_history"] else []
    ...
    async def aggregate_node(self, state: ClassroomState) -> ClassroomState:
        ...
        for reply in self._student_replies:
            state["transcript"].append(Message(role="student", speaker_id=reply.speaker_id, content=reply.content, timestamp_seconds=state["elapsed_seconds"]))
            if reply.intent == "ask_question":
                state["pending_questions"].append(PendingQuestion(speaker_id=reply.speaker_id, content=reply.content, created_at_seconds=state["elapsed_seconds"]))
            self._append_reply_events(state, reply)
            self._mark_taught_points(state, reply.content)
        if state["taught_points"] != board_before:
            state["pending_events"].append(self._next_event(state, BoardUpdateEvent, taught_points=sorted(state["taught_points"])))
        state["elapsed_seconds"] += 1
        return state

    async def persist_node(self, state: ClassroomState) -> ClassroomState:
        await self.checkpoint_store.save_checkpoint(state["session_id"], state, node="persist", event_seq=state["event_seq"])
        return state
    ...
    async def wait_node(self, state: ClassroomState) -> ClassroomState:
        if self.event_queue is not None:
            ...
            await self.checkpoint_store.save_checkpoint(state["session_id"], state, node="wait", event_seq=state["event_seq"])
        return state
    ...
    def _next_event(self, state: ClassroomState, cls: type, **kwargs: Any) -> AgentEventModel:
        state["event_seq"] += 1
        return cls(session_id=state["session_id"], event_seq=state["event_seq"], created_at=datetime.now(timezone.utc), **kwargs)
