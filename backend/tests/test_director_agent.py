...
STUDENTS = [
    Persona(id="s1", name="小红", personality="内向", knowledge_level="中等", behavior_traits="安静", interaction_frequency="low", attention_span="short"),
    Persona(id="s2", name="小明", personality="活泼", knowledge_level="中等", behavior_traits="积极", interaction_frequency="high", attention_span="short"),
    Persona(id="s3", name="小华", personality="认真", knowledge_level="优秀", behavior_traits="举手", interaction_frequency="medium", attention_span="long"),
]
...
async def test_twenty_turns_distribution_not_cluster_or_dead_air() -> None:
    ...
    agent = DirectorAgent(llm=llm, config=DirectorConfig(seed=1, speaker_cooldown_seconds=5))
    ...
    for i in range(20):
        elapsed = i * 10
        d = await agent.decide("谁想试试？", stage("p_middle"), STUDENTS, history, elapsed)
        assert d.actions
        ...
        for a in d.actions:
            if a.action_type in {"raise_hand", "speak"}:
                ...
                history.append(Message(role="student", speaker_id=a.speaker_id, content="发言", timestamp_seconds=elapsed))
...
async def test_p_lower_delay_shorter_and_more_daydream_than_high_school() -> None:
    ...
    for i in range(8):
        elapsed = i * 30
        ld = await lower.decide("继续听讲。", stage("p_lower"), STUDENTS, lower_history, elapsed)
        hd = await high.decide("继续听讲。", stage("h"), STUDENTS, high_history, elapsed)
        lower_ds.append(ld.next_action_delay_ms); high_ds.append(hd.next_action_delay_ms)
        ...
        for action in ld.actions:
            lower_history.append(Message(role="student", speaker_id=action.speaker_id, content="动作", timestamp_seconds=elapsed))
        for action in hd.actions:
            high_history.append(Message(role="student", speaker_id=action.speaker_id, content="动作", timestamp_seconds=elapsed))
...
async def test_called_student_must_speak_priority_5() -> None:
    agent = DirectorAgent(llm=mock_llm({"actions": [{"speaker_id": "s1", "action_type": "silent", "priority": 1}], "next_action_delay_ms": 999, "rationale": "x"}))
...
async def test_llm_code_block_can_parse() -> None:
    ...
    d = await DirectorAgent(llm=mock_llm(raw)).decide("谁会？", stage("p_middle"), STUDENTS, [], 1)
...
async def test_llm_name_is_normalized_and_multiple_speaks_downgraded() -> None:
    ...
    assert next(action for action in d.actions if action.speaker_id == "s2").action_type == "speak"


@pytest.mark.asyncio
async def test_invalid_json_fallback() -> None:
    d = await DirectorAgent(llm=mock_llm("oops"), config=DirectorConfig(seed=3)).decide("谁会？", stage("p_middle"), STUDENTS, [], 1)
    assert d.actions
...
async def test_unknown_speaker_and_illegal_action_filtered_or_corrected() -> None:
    payload = {"actions": [{"speaker_id": "ghost", "action_type": "speak", "priority": 5}, {"speaker_id": "s2", "action_type": "speak", "priority": 5}], "next_action_delay_ms": 1000, "rationale": "过滤"}
    hist = [Message(role="student", speaker_id="s2", content="刚说过", timestamp_seconds=9)]
    d = await DirectorAgent(llm=mock_llm(payload), config=DirectorConfig(speaker_cooldown_seconds=20)).decide("谁会？", stage("p_middle"), STUDENTS, hist, 10)
    ...
    bad = {"actions": [{"speaker_id": "s2", "action_type": "jump", "priority": 5}], "next_action_delay_ms": 1000, "rationale": "bad"}
    d2 = await DirectorAgent(llm=mock_llm(bad), config=DirectorConfig(seed=4)).decide("谁会？", stage("p_middle"), STUDENTS, [], 10)
