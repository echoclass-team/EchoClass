...
| 角色 | 代号 | 负责人 | 核心职责 | 代码领地 |
|---|---|---|---|---|
| **Agent 工程师** | `A-Agent` | TBD | LLM 客户端、学生 / Director / Evaluator Agent、RAG、LangGraph 状态机 | `backend/{agents,rag,llm,graph}` |
| **全栈工程师** | `B-Full` | TBD | 前端课堂 / 报告 UI、WebSocket、API 层、会话持久化、落地页与视觉打磨 | `frontend/`、`backend/{api,schemas,db}` |
...
|---|---|---|---|
| W1 | 单学生 Agent + 教案 RAG | 前端脚手架 + API 合约 | 立项书 v1 + 6 个人设 |
| W2 | Director 调度 + WebSocket 流式 | 虚拟课堂 UI + 会话管理 | 迷思库 v1（20 条）+ Rubric |
| W3 | 评估 Agent + 端到端 | 诊断报告页 | 迷思库 50 条 + 用户测试方案 |
